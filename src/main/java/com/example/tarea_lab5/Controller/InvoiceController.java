...
public class InvoiceController {
    ...
    public String save(
        ...
        if(invoice.getDate() != null &&
                invoice.getDate().isAfter(LocalDate.now())) {
        ...
        if(invoice.getType() != null &&
            ...
            if(invoice.getType().equals("FACTURA")
                    && !invoice.getCustomer()
                    .getDocumentType()
                    .equals("RUC")) {
            ...
            if(invoice.getType().equals("BOLETA")
                    && !invoice.getCustomer()
                    .getDocumentType()
                    .equals("DNI")) {
        ...
        for(InvoiceDetail d : invoice.getDetails()) {
            ...
            Product product = productRepository
                    .findById(d.getProduct().getId())
                    .orElseThrow();

            product.setStock(
                    product.getStock() - d.getQuantity()
            );

            productRepository.save(product);
        }
        invoice.getDetails().removeIf(d ->
                d.getProduct() == null ||
                        d.getProduct().getId() == null ||
                        d.getQuantity() == null ||
                        d.getQuantity() <= 0
